...
`--force` bypasses the daily cap. `--yes` is still required for send/call/buy/`numbers webhook` unless `CELL_AUTO_CONFIRM=1`. `--dry-run` does not charge and does not increment the daily cap. Usage is counted only after a successful live send or call.
