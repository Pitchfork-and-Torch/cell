...
cell status
cell doctor --offline
cell send +15551234567 "hello" --dry-run
...
| `cell doctor --offline` | Same checks, no carrier HTTP |
| `cell status` | Number, balance, trial flag |
| `cell numbers` | Owned numbers |
| `cell numbers search --area 512` | Find purchasable US locals |
| `cell numbers buy +1... --dry-run` | Preview a lease (no purchase) |
| `cell numbers buy +1... --yes` | Lease a number |
| `cell send +1... "text" --dry-run` | Validate SMS, no send |
| `cell send +1... "text" --yes` | Outbound SMS |
| `cell inbox [--with +1...]` | Recent messages |
| `cell inbox --local` | Sqlite cache only (no carrier) |
| `cell thread +1...` | One conversation |
| `cell watch --once` | Single poll, then exit |
| `cell watch` | Poll tail (long-running) |
| `cell webhook` | Local inbound HTTP server (long-running) |
| `cell tunnel` | Print cloudflared/ngrok helper |
| `cell numbers webhook https://.../sms --yes` | Pin inbound URL |
| `cell call +1... --say "..." --dry-run` | Validate call, no dial |
| `cell call +1... --say "..." --yes` | Outbound voice |

Buy, send, call, and `numbers webhook` are gated. Pass `--yes` only for the action you meant. `--dry-run` never calls the carrier. `--json` works before or after the subcommand.

## Tests

From the repo root (no carrier, no live number):
